...
import { LogOut, LayoutDashboard, Users, User as UserIcon, Menu } from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 flex flex-col md:flex-row">
      ...
      <header className="md:hidden flex items-center justify-between p-4 border-b border-slate-800 bg-slate-900/50 backdrop-blur-sm sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-white tracking-wide">StarterKit</span>
        </div>
        <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
          ...
          <SheetContent side="left" className="bg-slate-900 border-none p-0 w-72">
             <div className="flex items-center gap-3 p-6 border-b border-slate-800">
                ...
                <span className="font-bold text-white tracking-wide text-lg">StarterKit</span>
              </div>
            <NavigationLinks />
          </SheetContent>
        </Sheet>
      ...
      <aside className="hidden md:flex flex-col w-72 bg-slate-900 border-r border-slate-800 min-h-screen sticky top-0">
        <div className="flex items-center gap-3 p-6 border-b border-slate-800">
          ...
          <span className="font-bold text-white tracking-wide text-xl">StarterKit</span>
        </div>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900/50">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-64 bg-slate-900 border-slate-800 text-slate-200 pb-2 shadow-xl shadow-black/50 rounded-xl">
              <DropdownMenuLabel className="p-4 bg-slate-800/50 rounded-t-xl mb-2">
                ...
                <div className="mt-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-slate-800 text-slate-300 border border-slate-700">
                  Role: <span className={session?.user?.role === "admin" ? "text-indigo-400 ml-1 font-semibold" : "ml-1"}>{session?.user?.role}</span>
                </div>
              </DropdownMenuLabel>
