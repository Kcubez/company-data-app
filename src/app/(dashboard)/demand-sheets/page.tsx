...
function PipelineBar({
  ...
  pipeline?: { new: number; contacted: number; quoted: number; pending: number; closed: number };
}) {
  if (!pipeline) return null;
  const total = Object.values(pipeline).reduce((a, b) => a + b, 0);
  if (total === 0)
    return <div className="text-center py-6 text-slate-500 text-sm">No records yet</div>;
    ...
    <div className="space-y-4">
      ...
      <div className="flex h-8 rounded-xl overflow-hidden gap-0.5">
        {PIPELINE_STAGES.map(({ key, color }) => {
          const value = pipeline[key as keyof typeof pipeline];
          const pct = (value / total) * 100;
          if (pct === 0) return null;
          return (
            <div
              key={key}
              className={`${color} transition-all duration-700 ease-out flex items-center justify-center text-xs font-semibold text-white min-w-6`}
              style={{ width: `${pct}%` }}
              title={`${key}: ${value}`}
            >
              {pct > 8 ? value : ''}
      ...
      {activeTab === 'dashboard' && (
        <div className="space-y-6">
          ...
          <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
            ...
            <Card className="bg-slate-900 border-slate-800 shadow-lg">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-slate-400">Pending FU</CardTitle>
              </CardHeader>
              <CardContent>
                {statsLoading ? (
                  <Skeleton className="h-8 w-16 bg-slate-800" />
            ...
            <Card className="bg-slate-900 border-slate-800 shadow-lg">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-slate-400">Closed</CardTitle>
              </CardHeader>
              <CardContent>
                {statsLoading ? (
                  <Skeleton className="h-8 w-16 bg-slate-800" />
            ...
            {isLoading ? (
              ...
              data.records.map(record => (
                ...
                >
                  ...
                  <div className="md:col-span-2 min-w-0">
                    <p
                      className="text-xs leading-5 text-slate-400 line-clamp-3"
                      title={record.note}
                    >
                      {record.note}
                    </p>
